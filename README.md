...
*   **Loans Creation Form (`/`)**: A detailed onboarding interface with sections for Customer Info, Loan Details, Executive details, and Multi-Broker mapping. The **Add Loan** button remains disabled until all mandatory input fields are completed.
*   **Loan Details Screen (`/loan-detail`)**: Populated dynamically from the form submission. It computes ledger calculations in real-time, including:
    *   `Bill Comm Amt` (Sanctioned Amount × Commission Rate)
    *   `GST (18%)`
    *   `Invoice Amt`
    *   `TDS (10%)`
    *   `Net Receivable` (Invoice Amount − TDS Payouts)
*   **Disbursement Page (`/disbursement`)**: Renders KPI summaries, filter controls, full search query capability, and a paginated transaction grid. It is built to support horizontal table scroll handling on small viewports so the layout remains clean.

### 2. Task 2: Original Customer Onboarding Flow (`/task2/onboarding`)
...
*   **Implementation**: Accessible via the sidebar under **Onboard Customer** (routes `/task2/onboarding` and `/task2/review`).
*   **Features**:
